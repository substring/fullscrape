...
do
  ...
  then
    echo -e "\n+++ Scraping $system"
    $SCRAPER $updateMode $arcade $imgparms -rom_dir="$device" -output_file="$GAMELISTPATH/$system/gamelist.xml" -workers=$nbworkers -img_workers=1 -image_path="./downloaded_images" -image_dir="$IMAGESPATH/$system/downloaded_images"
  else
